  ...
  try {
    ...
    ****/
    ...
    if(ifilinput.is_open()){
      ...
      if(npars < 4){
	std:: cout << " npars smaller than 4 " << std:: endl;
    ...
    std:: cout << "    EXTRA PARAMETER FILES ... ... ... " << std:: endl;
    ...
    cbl::cosmology::Cosmology cosmology {cbl::cosmology::CosmologicalModel::_WMAP9_};        
    // as used to comupute the stacked shear profiles
    std:: cout << "s8 EH  _WMAP9_ " << cosmology.sigma8_Pk("EisensteinHu",0.0) << std:: endl;    
    ...
    // std:: cout << "s8 CAMB  " << cosmology.sigma8_Pk("CAMB",0.0) << std:: endl;    
